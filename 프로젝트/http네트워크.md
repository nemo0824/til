...
어떤 이미지 요청시 서버에서는 예시)1.1M 를 보내준다 HTTP헤더 0.1 HTTPBody: 1.0M

1. 첫번쨰 요청시 =⇒ 1.1M 
2. 두번째 요청시 =⇒ 1.1M

캐시 미적용시  이렇게 계속 다운로드 받게된다 

=⇒ 즉 인터넷 접근이 느리다(계속 같은자료임에도 불구하고 다운로드받아야해서),  비싸다(가격적으로)

1. 첫번째 요청시 =⇒ 1.1M 다운로드 및 브라우저캐시에 저장한다 (cache-control : max-age = 60) 
2. 두번쨰 요청시 =⇒ 60초이내 요청하게되면 유효하기때문에 브라우저캐시에서 바로 가져옴
3. 세번째 요청시 =⇒ 60초이후  다시 요청해서 다운로드받음

캐시 적용시  다운로드 받을필요없음

캐시 덕분에 캐시 가능 시간동안 네트워크를 사용하지않아도된다
...
1. 서버에서 기존 데이터를 변경한경우 
2. 서버에서 기존 데이터를 변경하지 않은 경우 =⇒ 검증헤더로 변경되지않았다면 HTTP 헤더만 전송하여서 기존에있던 브라우저캐시를 사용한다 즉 0.1M 만사용하게되는것



host 헤더 

http 에서 host 헤더는 필수이다

같은 ip 주소에서 여러개의 도메인을 호스팅할수있도록 서버가 요청을 구별하는데 사용된다  이기능은 가상 호스팅이라고한다

왜 host 헤더가 필요할까?
같은  ip에서 여러 도메인들을 호스팅 한다 

하나의 서버에서 여러 도메인을 제공할수있다 

클라이언트가 서버로 요청을 보낼떄 어떤 도메인의 리소스를 요청하는지 서버가 알아야하기때문에 

host 헤더가 이정보를 서버에 제공한다

포트 번호로는 구분분 불가능한 경우

포트 번호는 네트워크 연결의 출입구 역할을 하지만 여러 도메인이 같은 포트를 사용할수 있어서 

포트 번호만으로는 구분을 할수없어서 host 헤더가 필요한다 

가상 호스팅

host 헤더는 가상 호스팅을 구현하는 핵심요소

가상 호스팅이란?

하나의 서버(ip)에서 여러 도메인을 호스팅하는 기술

http 요청이 host 헤더를 기반으로 도메인을 구별 

포트 번호와 host 의 관계 

포트번호는 네티워크 계층에서 사용하는 요소로 동일한 IP주소에서 특정 애플리케이션을 구별하는데 사용된다

HTTP 프로토콜 레벨에서 도메인을 구별하는데는 포트번호ㄹ만으로 충분하지않다

웹은 기본적으로 80 443 포트를 사용

여러 도메인이 동일한 포트에 있을수있기때문에 포트번호만으로는 힘들다 

Host 헤더로 도메인별로 요청을 처리할수있다
